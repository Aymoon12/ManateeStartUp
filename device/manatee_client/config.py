...
class Config:
    ...
    min_positive_clips: int = 15              # >= N positive clips (~2s of sustained call) → detection
    high_confidence_threshold: float = 1.01  # disables the single-clip escape hatch (sigmoid can hit 1.0 on OOD noise)
    ...
    def load(cls, path: str = DEFAULT_CONFIG_PATH) -> "Config":
        ...
        return cls(
            ...
            min_positive_clips=int(_get(file_values, "MANATEE_MIN_POSITIVE_CLIPS", "15")),
            high_confidence_threshold=float(
                _get(file_values, "MANATEE_HIGH_CONFIDENCE_THRESHOLD", "1.01")
            ),
